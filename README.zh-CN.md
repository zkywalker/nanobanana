...
`init` 默认引导配置 GPT Image 2 + OpenAI-compatible 图像网关。你选择或已经配置了其他 provider 后，后续生成会尊重你的默认 profile/model。
...
| `--n <count>` | OpenAI 图像接口一次生成 / 编辑的输出张数 |
| `--openai-size <value>` | OpenAI 图像接口原生尺寸参数 |
| `--quality <value>` | Provider 原生质量档位 |
| `--background <value>` | Provider 原生背景选项 |
| `--output-format <value>` | Provider 原生输出格式，如 `png`、`jpeg`、`webp` |
| `--output-compression <value>` | Provider 支持时的原生输出压缩参数 |
| `--response-format <value>` | 可选 legacy OpenAI Images 响应格式覆盖：`url` 或 `b64_json` |
| `--timeout <seconds>` | 单次 OpenAI 图像请求超时覆盖 |
...
也可以设置 `BANANAHUB_IMAGE_TIMEOUT=<秒数>` 覆盖 OpenAI 图像请求超时；默认是 `900` 秒。单次调用可以用 `--timeout <秒数>`。
...
|---|---|---:|---:|---:|---|
| `openai-compatible` | 默认推荐，OpenAI 风格图像网关 | ✅ | ✅ | ✅ | GPT Image 2 路线；标准 Images API 能力取决于网关实现 |
| `openai` | OpenAI GPT Image 官方接口 | ✅ | ✅ | ✅ | GPT Image native 路线；支持多输出和 OpenAI Images API 多图编辑 |
| `google-ai-studio` | Gemini / Nano Banana 用户 | ✅ | ✅ | — | Gemini Developer API 路线 |
